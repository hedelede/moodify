...
function makeplot() {
    Plotly.d3.csv("actualdata4.csv", function (data) { processData(data) });
...
function makePlotly(t, I, V, P, standard_deviation) {
    ...
    //Current plot name
    Plotly.newPlot('currentGraph', trace1,
        { title: 'Current vs Time' }, { displayModeBar: false });
    //Voltage plot name
    Plotly.newPlot('voltageGraph', trace2,
        { title: 'Voltage vs Time' }, { displayModeBar: false });
    //Power plot name
    Plotly.newPlot('powerGraph', trace3,
        { title: 'Power vs Time' }, { displayModeBar: false });
};

//Call the plot function 5 times every second
window.setInterval(function () {
    makeplot();
}, 1000); // <----Calls function at every delay, in milliseconds (200 = Function called 5 times per second)