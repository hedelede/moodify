...
with open('data.csv', 'a') as csvfile:
	...
	spamwriter.writerow(["time", "voltage", "current", "power"])
	csvfile.close()
...
while True:
	...
	count = count + 1
	cost = cost + (0.25*power)
	with open('data.csv', 'a') as csvfile:
		spamwriter = csv.writer(csvfile, delimiter=',')
		spamwriter.writerow([count, str(voltage), str(current), str(power), str(cost)])
		csvfile.close()
	print(datetime.datetime.now(), current, voltage, power, cost)
ser.close()
